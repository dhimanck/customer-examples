...
Capture the current validated state of your Habitat-managed services before making infrastructure cha  nges.

Export Current Cluster State:

Bash
chef-automate config show > automate_ha_pre_mtls_backup.toml
Verify FQDN: Ensure the fqdn defined in your configuration matches the DNS record intended for your GCP ALB.
...
Extract Service Identity Certificates:
...
Establish ServerTlsPolicy (Strict Enforcement):
...
Upload Existing Service Identity to GCP:
...
Switch the internal Primary and Secondary LBs to TCP Mode for the Chef Server API. This allows the mTLS handshake from the GCP ALB to reach the Habitat-managed Nginx on the nodes without interference.
...
real_ip_recursive on;
"""
Apply: chef-automate config patch mtls_config.toml
...
Distribute Certificates: Ensure certificates are placed in standard directories.
...
Rollback (if needed to be executed on Bastion Host)
